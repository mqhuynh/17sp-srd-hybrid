    ...
    if($info_id != NULL){
        foreach ($all_data as $record)
        {
            if($record[0] == $info_id){
                ...
                {
                    if(strlen($data) == NULL){
                        echo $fieldname[$i].': <br><hr>' ;

                    }


                    else if(strpos($datatype[$i],"name") !== false){
                        echo '<h3>'.$fieldname[$i].': '.$data.'</h3>';
                    }
                    ...
                    // USE THE ACTUAL TEXT RATHER THAN .linkify($data).
                        // echo '<a target="_blank" href="'.linkify($data).'">Chương trình thờ phượng</a>';
                        ...
                        if(is_good_link($data)){
                            ...
                            if(strpos($data,".") != false){
                                ...
                                echo '<embed src="http://www.youtube.com/embed/'.$data.'"">';
                            }
                            echo '</div>';
                        }
